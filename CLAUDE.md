...
6. **네이티브 앱 OTA 배포** — 5번을 수행했다면(즉 `src/` 변경이 있었다면) **반드시 같이** 아래도 수행. 둘은 별개 배포 채널이라 5번만 하면 웹은 갱신되지만 앱(OTA)은 갱신되지 않음.
   1. **`package.json`의 `version`을 patch 단위로 올린다 (예: 0.2.0 → 0.2.1).** ⚠️ 이 단계를 빠뜨리면 `api/updates.js`가 새/구 버전을 같은 문자열로 비교해 "업데이트 없음"으로 응답 — zip 내용이 바뀌어도 앱에 영원히 반영되지 않는 조용한 실패가 난다 (2026-06-24 실제 발생, `decisions.md` 참고)
   2. `npm run ota:publish` 실행 → `public/ota/bundle-*.zip` + `latest.json` 갱신
   3. `git add package.json public/ota && git commit` 후 `git push`
   4. `npx vercel --prod` 재배포 (OTA 파일도 `/ota/...`로 서빙되므로 5번에서 이미 배포했어도 OTA 파일 갱신분을 반영하려면 한 번 더 배포 필요)
   5. 배포 후 `curl -s https://timerge.vercel.app/ota/latest.json`으로 version이 올린 값과 일치하는지 확인
   6. 네이티브 코드(플러그인 추가 등) 변경이 있었던 경우엔 OTA로 처리 불가 — `npm run apk:publish`로 APK도 갱신
