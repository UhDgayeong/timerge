...
5. `gh issue list --state open` 확인 후, **이번 작업으로 해결된 이슈가 있으면** 클로즈 처리
   - 커밋 메시지에 `Closes #N` 포함하면 push 시 GitHub이 자동 클로즈 (권장)
   - 이미 push한 경우: `gh issue close N --comment "커밋 SHA에서 해결. <커밋 URL>"` 로 수동 처리
