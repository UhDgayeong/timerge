...
**이유**: ① 상단 카드에 정보가 몰려 있어 무엇을 의미하는지 직관적으로 파악하기 어렵다는 피드백 — 정보를 실제로 해당하는 날짜의 카드로 옮겨 맥락을 일치시킴. ② `remainingMinutes` = 목표 − 실적 − (확정되지 않은 날들의 계획/목표 추정치)이므로, 이전 평일에 아직 계획/부분입력만 있고 실제 기록이 안 끝난 상태라면 역산값은 "추정에 기반한 추정"이 되어 신뢰도가 낮음. 모든 이전 평일이 실적으로 확정된 뒤에야 remainingMinutes가 확정값이 되고, 그때의 역산만 사용자에게 의미 있는 숫자가 됨.

**추가로 함께 처리한 두 가지 구분 문제**:
- 목표(예정, 아직 실적 없음) 시간 표시와 실제 기록된 시간 표시가 똑같은 스타일이라 구분이 안 된다는 피드백 → 예정 시간은 이탤릭+연한 색 + "목표" 접미사(`day-card__clock--planned`), 타입 배지도 실적 없으면 "근무 예정"처럼 ' 예정' 접미사 추가.
- 마지막 근무일이 아닌 날도 출근 시각만 입력하고 저장하면 "HH:MM 출근"이 보이도록 함 (기존엔 마지막 근무일에서만 노출).

**관련 파일**: `src/components/WeekHeader.tsx`(배너+안내 말풍선 텍스트 제거), `src/components/DayCard.tsx`(`departureInfo` prop, `isPartialClockIn`/`isPlannedPreview` 분기), `src/components/WeekView.tsx`(`priorDaysConfirmed` 조건 계산), `src/index.css`(`.day-card__departure`, `.day-card__clock--planned`)
