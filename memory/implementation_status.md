...
1. **마지막 근무일 퇴근 시각 역산** — 금요일이 마지막 근무일 전제. 출근 입력 시 헤더에 "N시 이후 퇴근 가능". 오전반차면 점심 흡수 후 오후 근무 full 역산. 오후반차면 퇴근 시각 고정 → "최대 N시까지 출근해야" 역산. (이슈 #3)
2. **Phase 1.5** — OCR 입력 (Tesseract.js 웹, ML Kit 모바일, 이슈 #4)
